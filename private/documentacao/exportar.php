<?php
// Exporta a listagem de documentação em CSV, JSON ou PDF
...
try {
    ...
    $dados = $ligacao->query(
        "SELECT d.tipo, d.nome, e.nome AS equipamento, d.data_documento,
                d.data_validade, d.ficheiro_nome
         FROM documento d
         LEFT JOIN equipamento e ON d.id_equipamento = e.id"
...
if ($formato === 'csv') {
    ...
    fputcsv($out, ['Tipo', 'Nome', 'Equipamento', 'Data Documento', 'Data Validade', 'Ficheiro'], ';');
...
if ($formato === 'json') {
    ...
    header('Content-Disposition: attachment; filename="documentacao.json"');
...
if ($formato === 'pdf') {
    ...
    <head>
        <meta charset="UTF-8">
        <title>Documentação — MedStock</title>
        <style>
    ...
    <body>
        ...
        <h2>MedStock — Documentação</h2>
        <p class="info">Exportado em: <?= date('d/m/Y H:i') ?> &nbsp;|&nbsp; Total: <?= count($dados) ?> registos</p>
        <table>
            <thead>
                <tr>
                    <th>Tipo</th><th>Nome</th><th>Equipamento</th>
                    <th>Data Documento</th><th>Data Validade</th><th>Ficheiro</th>
                </tr>
            </thead>
            <tbody>
