        ...
        try {
            ...
            $stmt = $pdo->prepare(
                "UPDATE fornecedor SET nome=?, nif=?, telefone=?, email=?, morada=?, website=?,
                 ...
                 tipo=(SELECT nome FROM tipos_fornecedor WHERE id=?), id_tipo_fornecedor=?,
...
<main class="conteudo">
    ...
    <form class="shadow p-4 rounded" style="max-width: 800px;" method="POST" action="editar.php?id=<?= $idEncrypted ?>" novalidate autocomplete="off">
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="id_tipo_fornecedor" required>
                ...
                <?php foreach ($tipos_fornecedor as $op) : ?>
                    <option value="<?= $op->id ?>" <?= ($fornecedor->id_tipo_fornecedor ?? '') == $op->id ? 'selected' : '' ?>><?= htmlspecialchars($op->nome) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="mb-3">
        ...
        <div class="d-flex justify-content-between mt-4">
            ...
            <button type="submit" class="btn btn-warning"><i class="fa-solid fa-pen"></i> Guardar Alterações</button>
        </div>
    </form>
</main>
