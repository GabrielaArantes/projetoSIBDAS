<?php
// Formulário de inserção de fornecedor com validação de NIF, telefone e email
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($erros)) {
        try {
            $pdo = get_pdo();
            $stmt = $pdo->prepare(
                "INSERT INTO fornecedor (nome, nif, telefone, email, morada, website, pessoa_contacto, telefone_contacto, tipo, id_tipo_fornecedor, observacoes)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, (SELECT nome FROM tipos_fornecedor WHERE id=?), ?, ?)"
            );
            $stmt->execute([
                $nome_empresa, $nif, $telefone, $email, $morada, $website,
                $pessoa_contacto, $tel_contacto, $id_tipo, $id_tipo, $observacoes
            ]);
    ...
    <form class="shadow p-4 rounded" style="max-width: 800px;" method="POST" action="inserir.php" novalidate>
        <div class="row mb-3">
            <div class="col">
                ...
                <input type="text" class="form-control" name="nome_empresa" value="<?= htmlspecialchars($_POST['nome_empresa'] ?? '') ?>" required>
            </div>
            <div class="col">
                <label class="form-label">NIF <span class="text-danger">*</span></label>
                <input type="text" class="form-control" name="nif" value="<?= htmlspecialchars($_POST['nif'] ?? '') ?>" required>
            </div>
        </div>
        <div class="row mb-3">
            <div class="col">
                <label class="form-label">Telefone <span class="text-danger">*</span></label>
                <input type="text" class="form-control" name="telefone" value="<?= htmlspecialchars($_POST['telefone'] ?? '') ?>" required>
            </div>
            <div class="col">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" name="morada" value="<?= htmlspecialchars($_POST['morada'] ?? '') ?>">
        </div>
        <div class="mb-3">
            <label class="form-label">Website</label>
            <input type="text" class="form-control" name="website" value="<?= htmlspecialchars($_POST['website'] ?? '') ?>">
        </div>
        <hr>
        <div class="row mb-3">
            <div class="col">
                <label class="form-label">Pessoa de Contacto</label>
                <input type="text" class="form-control" name="pessoa_contacto" value="<?= htmlspecialchars($_POST['pessoa_contacto'] ?? '') ?>">
            </div>
            <div class="col">
                <label class="form-label">Telefone da Pessoa de Contacto</label>
                <input type="text" class="form-control" name="telefone_contacto" value="<?= htmlspecialchars($_POST['telefone_contacto'] ?? '') ?>">
            </div>
        </div>
        <div class="mb-3">
            <label class="form-label">Tipo de Fornecedor <span class="text-danger">*</span></label>
            <select class="form-select" name="id_tipo_fornecedor" required>
                <option value="">Selecione...</option>
                <?php foreach ($tipos_fornecedor as $op) : ?>
                    <option value="<?= $op->id ?>" <?= (($_POST['id_tipo_fornecedor'] ?? '') == $op->id) ? 'selected' : '' ?>><?= htmlspecialchars($op->nome) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="mb-3">
        ...
        <div class="d-flex justify-content-between mt-4">
            ...
            <button type="submit" class="btn btn-success"><i class="fa-solid fa-check"></i> Guardar Fornecedor</button>
        </div>
    </form>
</main>
