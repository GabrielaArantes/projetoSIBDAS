...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($fornecedoresEscolhidos)) $erros[] = 'É obrigatório associar pelo menos um fornecedor.';

    // Verificar número de série único (excluindo o próprio equipamento)
    if (empty($erros)) {
        try {
            $pdo = get_pdo();
            $stmtCheck = $pdo->prepare("SELECT id FROM equipamento WHERE num_serie = ? AND equipamento_ativo = 1 AND id != ?");
            $stmtCheck->execute([trim($_POST['numero_serie']), $id]);
            if ($stmtCheck->fetch()) {
                $erros[] = "Já existe um equipamento ativo com o Número de Série '" . htmlspecialchars($_POST['numero_serie']) . "'. O número de série deve ser único.";
            }
        } catch (PDOException $err) {}
    }
    ...
    if (empty($erros)) {
        try {
            ...
            $stmt->execute([
                ...
                $_POST['data_aquisicao'] ?: null, $_POST['ano_fabrico'] ?: null, $custoAquisicao,
        ...
        if (!empty($garantia_inicio) || !empty($garantia_fim)) {
            try {
                ...
                $erro = "Erro ao guardar a garantia: " . $err->getMessage();
            }
        }
    ...
    <form method="POST" action="editar.php?id=<?= $idEncrypted ?>" enctype="multipart/form-data" novalidate autocomplete="off" class="shadow p-4 rounded bg-white" style="max-width: 900px; margin: auto;">
        ...
        <div class="tab-content">
            ...
            <div class="tab-pane fade" id="garantia" role="tabpanel">
                <div class="p-3 border rounded bg-light">
                    <h5 class="fw-bold mb-3">Adicionar Garantia / Contrato</h5>
